...
//Найти все рейсы в указанный временной отрезок(отрезки создать)
...
public class Main {
  static  Airport airport = new Airport("Sheremet");
  public static void main(String[] args) {
        ...
        /*airport
                ...
                .flatMap(x -> x.getFlightList()
                        .stream())
                .filter(x -> x.getType()
                        .equals(Type.ARRIVAl));


        airport.getTerminalList().stream().flatMap(x -> x.getFlightList()
                        .stream())
